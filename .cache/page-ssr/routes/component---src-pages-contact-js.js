...
var createServerContext = function createServerContext(name) {
...
/* harmony export */   findPath: () => (/* binding */ _findPath),
...
var trimPathname = function trimPathname(rawPathname) {
  var newRawPathname = rawPathname;
  var queryIndex = rawPathname.indexOf("?");
  if (queryIndex !== -1) {
    var _rawPathname$split = rawPathname.split("?"),
      _rawPathname$split2 = _slicedToArray(_rawPathname$split, 2),
      path = _rawPathname$split2[0],
      qs = _rawPathname$split2[1];
    newRawPathname = "".concat(path, "?").concat(encodeURIComponent(qs));
  }
  var pathname = decodeURIComponent(newRawPathname);
...
var findMatchPath = function findMatchPath(rawPathname) {
  ...
  var path = (0,_gatsbyjs_reach_router__WEBPACK_IMPORTED_MODULE_0__.pick)(pickPaths, trimmedPathname);
  if (path) {
...
// `/foo?bar=far` => `/page1`
var _findPath = function findPath(rawPathname) {
  ...
  if (redirect) {
    return _findPath(redirect.toPath);
  }
...
var prefetchPathname = _loader__WEBPACK_IMPORTED_MODULE_0__["default"].enqueue;
function graphql() {
...
var createPageDataUrl = function createPageDataUrl(rawPath) {
  var _rawPath$split = rawPath.split("?"),
    _rawPath$split2 = _slicedToArray(_rawPath$split, 2),
    path = _rawPath$split2[0],
    maybeSearch = _rawPath$split2[1];
  var fixedPath = path === "/" ? "index" : stripSurroundingSlashes(path);
  return "".concat("", "/page-data/").concat(fixedPath, "/page-data.json").concat(maybeSearch ? "?".concat(maybeSearch) : "");
};
 ...
 */
var shouldAbortFetch = function shouldAbortFetch(rawPath) {
...
var BOT_REGEX = /bot|crawler|spider|crawling/i;
...
var BaseLoader = /*#__PURE__*/function () {
  ...
  return _createClass(BaseLoader, [{
        ...
        if (status === 200) {
          try {
            ...
            if (maybeSearch && !jsonPayload.path.includes(maybeSearch)) {
              jsonPayload.path += "?".concat(maybeSearch);
            }
    ...
    value: function fetchPartialHydrationJson(loadObj) {
      var _this3 = this;
      var pagePath = loadObj.pagePath,
        _loadObj$retries2 = loadObj.retries,
        retries = _loadObj$retries2 === void 0 ? 0 : _loadObj$retries2;
      var url = createPageDataUrl(pagePath).replace(".json", "-rsc.json");
      return this.memoizedGet(url).then(function (req) {
        var status = req.status,
          responseText = req.responseText;
        ...
        if (status === 404 || status === 200) {
          ...
          // Need some code here to cache the 404 request. In case
          // multiple loadPageDataJsons result in 404s
          return _this3.fetchPartialHydrationJson(Object.assign(loadObj, {
            pagePath: "/404.html",
            notFound: true
          }));
        }
    ...
    value: function loadPageDataJson(rawPath) {
      ...
      return this.fetchPageDataJson({
        ...
        _this4.pageDataDb.set(pagePath, pageData);
        return pageData;
      });
    }
  }, {
    key: "loadPartialHydrationJson",
    value: function loadPartialHydrationJson(rawPath) {
      var _this5 = this;
      var pagePath = (0,_find_path__WEBPACK_IMPORTED_MODULE_3__.findPath)(rawPath);
      if (this.partialHydrationDb.has(pagePath)) {
        var pageData = this.partialHydrationDb.get(pagePath);
        if (true) {
          return Promise.resolve(pageData);
        }
      }
      return this.fetchPartialHydrationJson({
        ...
        _this5.partialHydrationDb.set(pagePath, pageData);
        return pageData;
      });
    }
  }, {
    key: "loadSliceDataJson",
    value: function loadSliceDataJson(sliceName) {
      var _this6 = this;
      if (this.slicesDataDb.has(sliceName)) {
        var jsonPayload = this.slicesDataDb.get(sliceName);
        return Promise.resolve({
          sliceName: sliceName,
          jsonPayload: jsonPayload
        });
      }
      var url = "".concat("", "/slice-data/").concat(sliceName, ".json");
      return doFetch(url, "GET").then(function (res) {
        var jsonPayload = JSON.parse(res.responseText);
        _this6.slicesDataDb.set(sliceName, jsonPayload);
        return {
          sliceName: sliceName,
          jsonPayload: jsonPayload
        };
    ...
    value: function loadPage(rawPath) {
      ...
      var loadDataPromises = [this.loadAppData(), this.loadPageDataJson(pagePath)];
      if (false) {}
      var inFlightPromise = Promise.all(loadDataPromises).then(function (allData) {
        var _allData = _slicedToArray(allData, 3),
          appDataResponse = _allData[0],
          pageDataResponse = _allData[1],
          rscDataResponse = _allData[2];
        ...
        var pageData = pageDataResponse.payload;
        var _pageData = pageData,
          componentChunkName = _pageData.componentChunkName,
          _pageData$staticQuery = _pageData.staticQueryHashes,
          pageStaticQueryHashes = _pageData$staticQuery === void 0 ? [] : _pageData$staticQuery,
          _pageData$slicesMap2 = _pageData.slicesMap,
          slicesMap = _pageData$slicesMap2 === void 0 ? {} : _pageData$slicesMap2;
        var finalResult = {};
        var dedupedSliceNames = Array.from(new Set(Object.values(slicesMap)));
        var loadSlice = function loadSlice(slice) {
          if (_this7.slicesDb.has(slice.name)) {
            return _this7.slicesDb.get(slice.name);
          } else if (_this7.sliceInflightDb.has(slice.name)) {
            return _this7.sliceInflightDb.get(slice.name);
          }
          var inFlight = _this7.loadComponent(slice.componentChunkName).then(function (component) {
            return {
          ...
          _this7.sliceInflightDb.set(slice.name, inFlight);
          inFlight.then(function (results) {
            _this7.slicesDb.set(slice.name, results);
            _this7.sliceInflightDb["delete"](slice.name);
          });
          return inFlight;
        };
        return Promise.all(dedupedSliceNames.map(function (sliceName) {
          return _this7.loadSliceDataJson(sliceName);
        })).then(function (slicesData) {
          var slices = [];
          var dedupedStaticQueryHashes = _toConsumableArray(pageStaticQueryHashes);
          for (var _i = 0, _Object$values = Object.values(slicesData); _i < _Object$values.length; _i++) {
            var _Object$values$_i = _Object$values[_i],
              jsonPayload = _Object$values$_i.jsonPayload,
              sliceName = _Object$values$_i.sliceName;
            slices.push(_objectSpread({
              name: sliceName
            }, jsonPayload));
            var _iterator = _createForOfIteratorHelper(jsonPayload.staticQueryHashes),
              _step;
            try {
              for (_iterator.s(); !(_step = _iterator.n()).done;) {
                var staticQueryHash = _step.value;
          ...
          var loadChunkPromises = [Promise.all(slices.map(loadSlice)), _this7.loadComponent(componentChunkName, "head")];
          if (true) {
            loadChunkPromises.push(_this7.loadComponent(componentChunkName));
          }
          ...
          var componentChunkPromises = Promise.all(loadChunkPromises).then(function (components) {
            var _components = _slicedToArray(components, 3),
              sliceComponents = _components[0],
              headComponent = _components[1],
              pageComponent = _components[2];
            finalResult.createdAt = new Date();
            var _iterator2 = _createForOfIteratorHelper(sliceComponents),
              _step2;
            try {
              for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                var sliceComponent = _step2.value;
            ...
            if (finalResult.status !== PageResourceStatus.Error) {
              ...
              if (_typeof(rscDataResponse === null || rscDataResponse === void 0 ? void 0 : rscDataResponse.payload) === "string") {
                ...
                return waitForResponse((0,react_server_dom_webpack__WEBPACK_IMPORTED_MODULE_0__.createFromReadableStream)(readableStream)).then(function (result) {
          ...
          var staticQueryBatchPromise = Promise.all(dedupedStaticQueryHashes.map(function (staticQueryHash) {
            // Check for cache in case this static query result has already been loaded
            if (_this7.staticQueryDb[staticQueryHash]) {
              var _jsonPayload = _this7.staticQueryDb[staticQueryHash];
              return {
                staticQueryHash: staticQueryHash,
                jsonPayload: _jsonPayload
              };
            }
            return _this7.memoizedGet("".concat("", "/page-data/sq/d/").concat(staticQueryHash, ".json")).then(function (req) {
            ...
            staticQueryResults.forEach(function (_ref) {
              ...
              _this7.staticQueryDb[staticQueryHash] = jsonPayload;
            });
            return staticQueryResultsMap;
          });
          return Promise.all([componentChunkPromises, staticQueryBatchPromise]).then(function (_ref2) {
            ...
            _this7.pageDb.set(pagePath, finalResult);
      ...
      inFlightPromise.then(function () {
        _this7.inFlightDb["delete"](pagePath);
    ...
    value: function prefetch(pagePath) {
      ...
      abortC.signal.addEventListener("abort", function () {
        var index = _this8.prefetchQueued.findIndex(function (_ref4) {
        ...
        if (index !== -1) {
          _this8.prefetchQueued.splice(index, 1);
        }
      });
      if (!this.isPrefetchQueueRunning) {
        this.isPrefetchQueueRunning = true;
        setTimeout(function () {
          _this8._processNextPrefetchBatch();
        }, 3000);
      }
      return {
    ...
    value: function _processNextPrefetchBatch() {
      var _this9 = this;
      var idleCallback = window.requestIdleCallback || function (cb) {
        return setTimeout(cb, 0);
      };
      idleCallback(function () {
        var toPrefetch = _this9.prefetchQueued.splice(0, 4);
        var prefetches = Promise.all(toPrefetch.map(function (_ref6) {
          var _ref7 = _slicedToArray(_ref6, 2),
            pagePath = _ref7[0],
            dPromise = _ref7[1];
          // Tell plugins with custom prefetching logic that they should start
          // prefetching this path.
          if (!_this9.prefetchTriggered.has(pagePath)) {
            _this9.apiRunner("onPrefetchPathname", {
              pathname: pagePath
            });
            _this9.prefetchTriggered.add(pagePath);
          }

          // If a plugin has disabled core prefetching, stop now.
          if (_this9.prefetchDisabled) {
            return dPromise.resolve(false);
          }
          return _this9.doPrefetch((0,_find_path__WEBPACK_IMPORTED_MODULE_3__.findPath)(pagePath)).then(function () {
            if (!_this9.prefetchCompleted.has(pagePath)) {
              _this9.apiRunner("onPostPrefetchPathname", {
                pathname: pagePath
              });
              _this9.prefetchCompleted.add(pagePath);
            }
            dPromise.resolve(true);
          });
        }));
        if (_this9.prefetchQueued.length) {
          prefetches.then(function () {
            setTimeout(function () {
              _this9._processNextPrefetchBatch();
            }, 3000);
          });
        } else {
          _this9.isPrefetchQueueRunning = false;
        }
      });
    }
  }, {
      ...
      if (false) {} else {
        return (0,_prefetch__WEBPACK_IMPORTED_MODULE_1__["default"])(pageDataUrl, {
          ...
          return (
            ...
            _this10.loadPageDataJson(pagePath)
          );
        });
      }
    }
    ...
    value: function loadAppData() {
      ...
      return this.memoizedGet("".concat("", "/page-data/app-data.json")).then(function (req) {
        ...
        if (status !== 200 && retries < 3) {
          ...
          return _this11.loadAppData(retries + 1);
        }
...
var ProdLoader = /*#__PURE__*/function (_BaseLoader2) {
  function ProdLoader(asyncRequires, matchPaths, pageData) {
    ...
    var loadComponent = function loadComponent(chunkName) {
      var exportType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "components";
      if (true) {
    ...
    _this12 = _callSuper(this, ProdLoader, [loadComponent, matchPaths]);
    if (pageData) {
      _this12.pageDataDb.set((0,_find_path__WEBPACK_IMPORTED_MODULE_3__.findPath)(pageData.path), {
  ...
  return _createClass(ProdLoader, [{
    key: "doPrefetch",
    value: function doPrefetch(pagePath) {
      return _superPropGet(ProdLoader, "doPrefetch", this, 3)([pagePath]).then(function (result) {
    ...
    key: "loadPageDataJson",
    value: function loadPageDataJson(rawPath) {
      return _superPropGet(ProdLoader, "loadPageDataJson", this, 3)([rawPath]).then(function (data) {
        if (data.notFound) {
    ...
    key: "loadPartialHydrationJson",
    value: function loadPartialHydrationJson(rawPath) {
      return _superPropGet(ProdLoader, "loadPartialHydrationJson", this, 3)([rawPath]).then(function (data) {
        if (data.notFound) {
          if (shouldAbortFetch(rawPath)) {
            return data;
          }
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (function (pathAndSearch) {
  ...
  var _pathAndSearch$split = pathAndSearch.split("?"),
    _pathAndSearch$split2 = _slicedToArray(_pathAndSearch$split, 2),
    path = _pathAndSearch$split2[0],
    _pathAndSearch$split3 = _pathAndSearch$split2[1],
    search = _pathAndSearch$split3 === void 0 ? "" : _pathAndSearch$split3;
  if (search) {
    search = "?" + search;
  }
  if (path === "/") {
...
function Slice(props) {
  if (true) {
    ...
    var internalProps = _objectSpread(_objectSpread({}, props), {}, {
      sliceName: props.alias
    });
    delete internalProps.alias;
    delete internalProps.__renderedByLocation;
    var slicesContext = (0,react__WEBPACK_IMPORTED_MODULE_0__.useContext)(_slice_context__WEBPACK_IMPORTED_MODULE_3__.SlicesContext);

    // validate props
    var propErrors = _validateSliceProps(props);
    ...
    if (slicesContext.renderEnvironment === "server") {
      ...
      var additionalContextMessage = "";

      // just in case generating additional contextual information fails, we still want the base message to show
      // and not show another cryptic error message
      try {
        additionalContextMessage = "\n\nSlice component \"".concat(slicesContext.sliceRoot.name, "\" (").concat(slicesContext.sliceRoot.componentPath, ") tried to render <Slice alias=\"").concat(props.alias, "\"/>");
      } catch (_unused) {
        // don't need to handle it, we will just skip the additional context message if we fail to generate it
      }
      throw new Error("Nested slices are not supported.".concat(additionalContextMessage, "\n\nSee https://gatsbyjs.com/docs/reference/built-in-components/gatsby-slice#nested-slices"));
    } else {
      throw new Error("Slice context \"".concat(slicesContext.renderEnvironment, "\" is not supported."));
    }
  } else {}
}
var SlicePropsError = /*#__PURE__*/function (_Error) {
  function SlicePropsError(inBrowser, sliceName, propErrors, renderedByLocation) {
    var _this;
    _classCallCheck(this, SlicePropsError);
    var errors = Object.entries(propErrors).map(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 2),
        key = _ref2[0],
        value = _ref2[1];
      return "not serializable \"".concat(value, "\" type passed to \"").concat(key, "\" prop");
    }).join(", ");
    var name = "SlicePropsError";
    var stack = "";
    var message = "";
    if (inBrowser) {
      // They're just (kinda) kidding, I promise... You can still work here <3
      //   https://www.gatsbyjs.com/careers/
      var fullStack = react__WEBPACK_IMPORTED_MODULE_0___default().__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactDebugCurrentFrame.getCurrentStack();

      // remove the first line of the stack trace
      var stackLines = fullStack.trim().split("\n").slice(1);
      stackLines[0] = stackLines[0].trim();
      stack = "\n" + stackLines.join("\n");
      message = "Slice \"".concat(sliceName, "\" was passed props that are not serializable (").concat(errors, ").");
    } else {
      // we can't really grab any extra info outside of the browser, so just print what we can
      message = "".concat(name, ": Slice \"").concat(sliceName, "\" was passed props that are not serializable (").concat(errors, ").");
      var _stackLines = new Error().stack.trim().split("\n").slice(2);
      stack = "".concat(message, "\n").concat(_stackLines.join("\n"));
    }
    _this = _callSuper(this, SlicePropsError, [message]);
    _this.name = name;
    if (stack) {
      _this.stack = stack;
    } else {
    ...
    if (renderedByLocation) {
      _this.forcedLocation = _objectSpread(_objectSpread({}, renderedByLocation), {}, {
        functionName: "Slice"
      });
    }
    return _this;
  }
  _inherits(SlicePropsError, _Error);
  return _createClass(SlicePropsError);
}(/*#__PURE__*/_wrapNativeSuper(Error));
var _validateSliceProps = function validateSliceProps(props) {
  ...
  for (var _i = 0, _Object$entries = Object.entries(props); _i < _Object$entries.length; _i++) {
    ...
    var propPath = path ? "".concat(path, ".").concat(name) : name;
    if (_typeof(value) === "function") {
      errors[propPath] = _typeof(value);
    } else if (_typeof(value) === "object" && seenObjects.indexOf(value) <= 0) {
      seenObjects.push(value);
      _validateSliceProps(value, errors, seenObjects, propPath);
    }
  }
  return errors;
};
...
var SlicesResultsContext = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createContext({});
var SlicesContext = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createContext({});
var SlicesMapContext = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createContext({});
var SlicesPropsContext = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createContext({});
...
var _excluded = ["sliceName", "allowEmpty", "children"];
...
var InlineSlice = function InlineSlice(_ref) {
  var sliceName = _ref.sliceName,
    allowEmpty = _ref.allowEmpty,
    children = _ref.children,
    sliceProps = _objectWithoutProperties(_ref, _excluded);
  var slicesMap = (0,react__WEBPACK_IMPORTED_MODULE_0__.useContext)(_context__WEBPACK_IMPORTED_MODULE_1__.SlicesMapContext);
  var slicesResultsMap = (0,react__WEBPACK_IMPORTED_MODULE_0__.useContext)(_context__WEBPACK_IMPORTED_MODULE_1__.SlicesResultsContext);
  var concreteSliceName = slicesMap[sliceName];
  var slice = slicesResultsMap.get(concreteSliceName);
  if (!slice) {
    if (allowEmpty) {
      return null;
    } else {
      throw new Error("Slice \"".concat(concreteSliceName, "\" for \"").concat(sliceName, "\" slot not found"));
    }
  }
  return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(slice.component, _extends({
...
var ServerSliceRenderer = function ServerSliceRenderer(_ref) {
  var sliceId = _ref.sliceId,
    children = _ref.children;
  var contents = [/*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("slice-start", {
    id: "".concat(sliceId, "-1")
...
var getSliceId = function getSliceId(sliceName, sliceProps) {
  if (!Object.keys(sliceProps).length) {
    return sliceName;
  }
  var propsString = (0,gatsby_core_utils_create_content_digest__WEBPACK_IMPORTED_MODULE_1__.createContentDigest)(sliceProps);
  return "".concat(sliceName, "-").concat(propsString);
};
var ServerSlice = function ServerSlice(_ref) {
  var sliceName = _ref.sliceName,
    allowEmpty = _ref.allowEmpty,
    children = _ref.children,
    sliceProps = _objectWithoutProperties(_ref, _excluded);
  var slicesMap = (0,react__WEBPACK_IMPORTED_MODULE_0__.useContext)(_context__WEBPACK_IMPORTED_MODULE_2__.SlicesMapContext);
  var slicesProps = (0,react__WEBPACK_IMPORTED_MODULE_0__.useContext)(_context__WEBPACK_IMPORTED_MODULE_2__.SlicesPropsContext);
  var concreteSliceName = slicesMap[sliceName];
  ...
  var sliceId = getSliceId(concreteSliceName, sliceProps);

  // set props on context object for static-entry to return
  var sliceUsage = slicesProps[sliceId];
  if (!sliceUsage) {
...
var StaticQueryContext = (0,_context_utils__WEBPACK_IMPORTED_MODULE_1__.createServerOrClientContext)("StaticQuery", {});
...
var warnedAboutStaticQuery = false;
...
function d(t, e) {
  ...
  return `${(0,gatsby_page_utils_apply_trailing_slash_option__WEBPACK_IMPORTED_MODULE_2__.applyTrailingSlashOption)(n, e)}${r}${o}`;
}
const m = (t, e) => "number" == typeof t ? t : l(t) ? u(t) ? function (t) {
    ...
    if (u(t)) return t;
    const r = _(),
      o = (0,_gatsbyjs_reach_router__WEBPACK_IMPORTED_MODULE_1__.resolve)(t, e);
...
const b = {
  ...
  partiallyActive: (prop_types__WEBPACK_IMPORTED_MODULE_3___default().bool)
};
function w(t) {
  /*#__PURE__*/return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(_gatsbyjs_reach_router__WEBPACK_IMPORTED_MODULE_1__.Location, null, ({
    location: n
  }) => /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(P, i({}, t, {
    _location: n
  })));
}
class P extends (react__WEBPACK_IMPORTED_MODULE_0___default().Component) {
  ...
  render() {
    ...
    return l(_) ? /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(_gatsbyjs_reach_router__WEBPACK_IMPORTED_MODULE_1__.Link, i({
      to: _,
      state: p,
      getProps: r,
    ...
    }, u)) : /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("a", i({
      href: _
    }, u));
  }
}
P.propTypes = i({}, b, {
  ...
  state: (prop_types__WEBPACK_IMPORTED_MODULE_3___default().object)
});
const E = /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().forwardRef((t, n) => /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(w, i({
    innerRef: n
  }, t))),
  g = (t, e) => {
...
/***/ "./node_modules/gatsby-plugin-modal-routing/ModalRoutingContext.js":
/*!*************************************************************************!*\
  !*** ./node_modules/gatsby-plugin-modal-routing/ModalRoutingContext.js ***!
  \*************************************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
/***/ "./node_modules/gatsby-plugin-modal-routing/ModalRoutingLink.js":
/*!**********************************************************************!*\
  !*** ./node_modules/gatsby-plugin-modal-routing/ModalRoutingLink.js ***!
  \**********************************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
var _ModalRoutingContext = _interopRequireDefault(__webpack_require__(/*! ./ModalRoutingContext */ "./node_modules/gatsby-plugin-modal-routing/ModalRoutingContext.js"));
...
/***/ "./node_modules/gatsby-plugin-modal-routing/index.js":
/*!***********************************************************!*\
  !*** ./node_modules/gatsby-plugin-modal-routing/index.js ***!
  \***********************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
var _ModalRoutingLink = _interopRequireDefault(__webpack_require__(/*! ./ModalRoutingLink */ "./node_modules/gatsby-plugin-modal-routing/ModalRoutingLink.js"));
exports.Link = _ModalRoutingLink["default"];
...
/* harmony import */ var prop_types__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! prop-types */ "./node_modules/prop-types/index.js");
/* harmony import */ var prop_types__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(prop_types__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _images_feather_sprite_svg__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../images/feather-sprite.svg */ "./src/images/feather-sprite.svg");
...
var FeatherIcon = function FeatherIcon(_ref) {
  ...
  }, restProps), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("use", {
    xlinkHref: "".concat(_images_feather_sprite_svg__WEBPACK_IMPORTED_MODULE_1__["default"], "#").concat(name)
  }));
};
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (FeatherIcon);
...
/***/ "./src/components/formDatatext.js":
/*!****************************************!*\
  !*** ./src/components/formDatatext.js ***!
  \****************************************/
...
if ((typeof window === "undefined" ? "undefined" : _typeof(window)) !== "undefined") {
  window.onload = function () {
    ...
    var formDataToJson = function formDataToJson(formData) {
      ...
      var dataObj = Array.from(entries).reduce(function (data, _ref) {
        var _ref2 = _slicedToArray(_ref, 2),
          key = _ref2[0],
          value = _ref2[1];
        data[key] = value;
    ...
    var postToFormspree = function postToFormspree(formData) {
      return fetch('https://formspree.io/mellpvwx', {
        method: 'POST',
    ...
    if (submit) {
      submit.addEventListener('submit', function (e) {
        ...
        postToFormspree(formData).then(function (response) {
...
/* harmony import */ var _formDatatext__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./formDatatext */ "./src/components/formDatatext.js");
/* harmony import */ var _formDatatext__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_formDatatext__WEBPACK_IMPORTED_MODULE_2__);



var Formtext = function Formtext() {
  return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("form", {
    action: "https://formspree.io/mellpvwx",
    name: "contactForm",
    id: "contactForm",
    method: "post",
...
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(react__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var gatsby__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! gatsby */ "./.cache/gatsby-browser-entry.js");
/* harmony import */ var gatsby_plugin_modal_routing__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! gatsby-plugin-modal-routing */ "./node_modules/gatsby-plugin-modal-routing/index.js");
/* harmony import */ var _components_formtext__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../components/formtext */ "./src/components/formtext.js");
/* harmony import */ var _components_FeatherIcon__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ../components/FeatherIcon */ "./src/components/FeatherIcon.js");





var ModalPage = function ModalPage() {
  return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(gatsby_plugin_modal_routing__WEBPACK_IMPORTED_MODULE_2__.ModalRoutingContext.Consumer, null, function (_ref) {
    var modal = _ref.modal,
      closeTo = _ref.closeTo;
    return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", null, modal ? /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(gatsby__WEBPACK_IMPORTED_MODULE_1__.Link, {
      to: closeTo
    }, "Close") : /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("header", null, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("h1", null, "Website Title")), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("h2", null, "Modal Page"), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "meka_hero"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("nav", {
      className: "top-bar topbar-responsive"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "top-bar-title"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("h1", {
      className: "hidden-h1"
    }, "meka creative"), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("a", {
      href: "https://www.mekacreative.io",
      onclick: "track('Click-logo-top','Click-logo-top')",
      rel: "canonical"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "icon-svg svgicon-meka_logo_app"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("img", {
      src: __webpack_require__(/*! ../images/mekacreative_logo.svg */ "./src/images/mekacreative_logo.svg")
    }))))), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "row"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      ...
      className: "h2_hero"
    }, "Welcome to your creative hiring assistant \uD83D\uDC4B")), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "column small-12"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("h3", {
      className: "h3_hero"
    }, "Artifical intelligence for creative profiles sourcing and recruiting")), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "column small-12"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "row"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "column small-6 medium-5"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement(_components_formtext__WEBPACK_IMPORTED_MODULE_3__["default"], null)))))), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "column small-12 medium-5 hero-img"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("img", {
      src: __webpack_require__(/*! ../images/wireframes_high_fi.svg */ "./src/images/wireframes_high_fi.svg")
    }))), /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("div", {
      className: "row align-bottom text-center bottom_hero"
    }, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_0___default().createElement("svg", {
      className: "white_angle",
      xmlns: "http://www.w3.org/2000/svg",
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ("/static/feather-sprite-3b181eb21cc39c609ffdbcfe1688d12d.svg");

/***/ }),

/***/ "./src/images/mekacreative_logo.svg":
/*!******************************************!*\
  !*** ./src/images/mekacreative_logo.svg ***!
  \******************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/***/ "./src/images/wireframes_high_fi.svg":
/*!*******************************************!*\
  !*** ./src/images/wireframes_high_fi.svg ***!
  \*******************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ("/static/wireframes_high_fi-675ac7860060dd5dab97d6948516e6c2.svg");
